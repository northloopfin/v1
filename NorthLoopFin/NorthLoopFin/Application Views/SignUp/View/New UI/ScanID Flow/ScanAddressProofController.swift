//
//  ScanAddressProofController.swift
//  NorthLoopFin
//
...
class ScanAddressProofController: BaseViewController {
    ...
    var typeData:SelectIDType = SelectIDType.init(type: AppConstants.SelectIDTYPES.ADDRESSPROOF, images: [])
    ...
    override func viewDidLoad() {
        ...
        if let vc = self.previousViewController {
            if vc.self.isKind(of: ScanVisaController.self){
    ...
    @IBAction func btnNext_pressed(_ sender: UIButton) {
        for controllersOnNavStack in self.navigationController!.viewControllers {
            if controllersOnNavStack.self.isKind(of: ScanIDMenuController.self){
                self.navigationController?.popToViewController(controllersOnNavStack, animated: true)
            }
        }
    }
