//
//  Colors.swift
//  NorthLoopFin
//
//  Created by Daffolapmac-19 on 28/12/18.
//  Copyright © 2018 NorthLoop. All rights reserved.
//

import Foundation
import UIKit
struct Colors{
    static let DustyGray155155155 = UIColor.init(red: 155, green: 155, blue: 155)
    static let PurpleColor17673149 = UIColor.init(red: 176, green: 73, blue: 149)
    static let Mercury226226226 = UIColor.init(red: 226, green: 226, blue: 226)
    static let Zorba161149133 = UIColor.init(red: 161, green: 149, blue: 133)
    static let Alto224224224 = UIColor.init(red: 224, green: 224, blue: 224)
    static let Taupe776857 = UIColor.init(red: 77, green: 68, blue: 57)
    static let Cameo213186154 = UIColor.init(red: 213, green: 186, blue: 154)
    static let MainTitleColor = UIColor.black
    static let Tundora747474 = UIColor.init(red: 74, green: 74, blue: 74)
    static let NeonCarrot25414966 = UIColor.init(red: 254, green: 149, blue: 66)
    static let whiteColor = UIColor.white
    static let TaupeGray131130131 = UIColor.init(red: 131, green: 130, blue: 131)
    static let ivory255254247 = UIColor.init(red: 255, green: 254, blue: 247)
    static let Amour244233239 = UIColor.init(red: 244, green: 233, blue: 239)

//    static let Mercury = UIColor.init(red: 226, green: 226, blue: 226)


}
